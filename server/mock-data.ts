...
export const MOCK_SCHEMAS: Record<string, { name: string; fullName: string; comment: string }[]> = {
  toyota_production: [
...
export const MOCK_SCHEMA_CONTEXT: Record<string, string> = {
...
/** Simulated query results keyed by pattern matches on SQL */
...
const MOCK_QUERY_RESULTS: MockResult[] = [
...
const DEFAULT_RESULT: MockResult = {
  pattern: /./,
...
export function getMockQueryResult(sql: string): { columns: string[]; rows: string[][]; rowCount: number; truncated: boolean } {
  const match = MOCK_QUERY_RESULTS.find(m => m.pattern.test(sql));
  const result = match || DEFAULT_RESULT;
  return {
...
export function getMockSchemaContext(schemas: string[]): string {
  return schemas
    .map(s => MOCK_SCHEMA_CONTEXT[s] || '')
    .filter(Boolean)
    .join('\n');
}

/** Get all available mock schema context (for NLP engine) */
export function getAllMockSchemaContext(): string {
  return Object.values(MOCK_SCHEMA_CONTEXT).join('\n');
}
